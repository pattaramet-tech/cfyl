...
export function calculateGroupStandings(params: CalculateGroupStandingsParams): GroupStandingsResult {
  ...
  const overridesByTeamId = new Map((params.overrides || []).map((o) => [o.teamId, o]));

  // Manual override (tournament_standing_overrides, already-approved table):
  // an overridden team's rank takes priority; teams without an override fill
  // the remaining positions in their computed relative order.
  const hasAnyOverride = overridesByTeamId.size > 0;
  let finalOrder = orderedWithExplanation;
  if (hasAnyOverride) {
    const overridden = orderedWithExplanation
      .filter((e) => overridesByTeamId.has(e.stats.teamId))
      .sort((a, b) => (overridesByTeamId.get(a.stats.teamId) as StandingOverrideInput).overrideRank - (overridesByTeamId.get(b.stats.teamId) as StandingOverrideInput).overrideRank);
    const nonOverridden = orderedWithExplanation.filter((e) => !overridesByTeamId.has(e.stats.teamId));

    // Interleave: place overridden teams at their exact rank, fill gaps with
    // non-overridden teams in their computed relative order.
    ...
    for (const entry of overridden) {
      const rank = (overridesByTeamId.get(entry.stats.teamId) as StandingOverrideInput).overrideRank;
      if (rank >= 1 && rank <= totalCount && slots[rank - 1] === null) {
        slots[rank - 1] = entry;
      }
    }
    let fillIndex = 0;
  ...
  const rows: StandingsRow[] = finalOrder.map((entry, index) => {
    ...
    const override = overridesByTeamId.get(entry.stats.teamId);
    ...
    return {
      ...
      qualificationStatus,
      tiebreakExplanation: override ? `จัดอันดับโดย Admin: ${override.reason}` : entry.explanation,
