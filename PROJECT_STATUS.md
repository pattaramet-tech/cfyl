...
- [x] /admin/teams + /admin/players: shared BulkImportPanel (editable grid + xlsx/csv import + preview + save valid)
- [x] lib/bulk-import: team + player validation (unique, league-division-required, team match by code→name, player_code auto-gen, dup protection) — unit-verified
- [x] API: teams/bulk/{preview,save,template} + players/bulk/{preview,save,template} (auth); audit 4 actions
- [x] friendly errors; division null supported; build ✅; single add/edit + groups/fixtures unaffected
