...
export default function FixturesByMatchdayPage() {
  const params = useParams();
  const year = String(params.seasonYear ?? '');
  const ageCode = String(params.ageGroupCode ?? '');
  const mdCode = String(params.matchdayCode ?? '');
    ...
    <div className="space-y-6">
      <div className="cfyl-section">
        ...
        <p className="text-sm text-slate-500 mt-1">
          CFYL {year} · {ageCode.toUpperCase()} · {mdCode.toUpperCase()}
        </p>
      </div>
      ...
      {state === 'loading' ? (
        ...
        <div className="cfyl-section text-center space-y-3">
          <p className="text-slate-600">ไม่พบหน้าที่ระบุ ({year}/{ageCode}/{mdCode})</p>
          <Link href="/fixtures" className="cfyl-btn-secondary">ไปหน้าโปรแกรมแข่งขัน</Link>
        </div>
      ) : (
        <FixturesView seasonId={resolved.seasonId} ageGroupId={resolved.ageGroupId} matchdayCode={mdCode} />
      )}
    </div>
  );
}
