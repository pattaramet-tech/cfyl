...
`tournament_matches.home_source_type`/`away_source_type` รองรับ 8 ค่า (ดู `TOURNAMENT_V2_DATA_MODEL.md` หมวด 2.8):
...
| `best_ranked` | ทีมอันดับดีที่สุดข้ามกลุ่ม | `RULE_CODE:SLOT` | `best_ranked \| third_place:1` |
| `bye` | ไม่มีคู่แข่ง (ผ่านอัตโนมัติ) | `BYE` | `bye \| BYE` |
| `tbd` | ยังไม่กำหนด | `TBD` | `tbd \| TBD` |

### กฎสำคัญ: แยก `group_slot` และ `group_rank` อย่างเด็ดขาด
