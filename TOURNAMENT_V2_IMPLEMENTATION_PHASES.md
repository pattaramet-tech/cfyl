...
- **Files Expected to Change**: `lib/tournament/standings/*.ts` (ไฟล์ใหม่ทั้งหมด รวม `calculateFairPlayScore.ts`, `rankCrossGroupCandidates.ts` ซึ่งมี `identifyG16ThirdPlaceCandidates`), `lib/tournament/services/standings.ts` (data-loading), `app/api/tournament/admin/standings/**`, `app/api/tournament/admin/qualification-draws/**` (มีอยู่แล้วจาก PR #7 — ไม่ใช่ไฟล์ execute-draw ใหม่), `app/api/tournament/public/standings/**`
