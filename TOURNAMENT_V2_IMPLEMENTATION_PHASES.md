...
- **Tests**: **บังคับ Unit Test ครบ Validation Matrix ทั้ง Error และ Warning ทุกข้อ** (ดู `TOURNAMENT_V2_SCHEDULING_AND_IMPORT.md` หมวด 7), Integration test Import ไฟล์เดิมซ้ำไม่สร้าง Match ซ้ำ (Idempotency ผ่าน `match_code`), Integration test Rollback Batch ลบเฉพาะ Match ที่ Batch นั้นสร้าง ไม่กระทบ Match อื่น, Integration test D-28: แก้ไข Match ที่ Publish แล้วโดยไม่ยืนยันต้องถูกปฏิเสธ (409) และ Match เดิมต้องไม่ถูกแตะต้อง, ยืนยันแล้วต้องเปลี่ยนเป็น `revision_required` เท่านั้น (ไม่ Auto-publish)
- **Acceptance Criteria**: Import ไฟล์ 100+ แถวได้ถูกต้อง แสดง Preview/Diff/Error/Warning ครบ; Import ซ้ำไฟล์เดิมไม่สร้างข้อมูลซ้ำ; Rollback Batch คืนสถานะก่อน Import ได้ถูกต้อง 100%; แก้ไขตารางที่ Publish แล้วไม่สามารถ Save ได้โดยไม่มีการยืนยันจาก `tournament_super_admin`
...
- **Scope**: Implement `calculateGroupStandings`, `resolveTournamentTiebreak`, `calculateFairPlayScore` (ใหม่ — D-06), `rankCrossGroupCandidates`, `rankBestThirdPlacedTeams` (`method='ranked'`), `identifyG16ThirdPlaceCandidates` (ใหม่ — `method='draw'`, D-29), `calculateQualificationStatus`, `applyManualQualificationOverride` (Section 7 ของแผนต้นทาง) — **DECISION LOCKED (D-09, 2026-07-14)**: ไม่มี Branch สำหรับผลเสมออีกต่อไป (ทุกนัดมีผู้ชนะเสมอ)
- **G-U16 Qualification Draw (D-29) — สำคัญ**: การจับฉลากเป็นกระบวนการที่ทำ **ด้วยมือ บนกระดาษ ที่หน้างาน** เท่านั้น ซอฟต์แวร์**ไม่มีฟังก์ชันสุ่มเลือกทีมใดๆ**ทั้งสิ้น ระบบมีหน้าที่เพียง **ระบุ (identify)** ทีมอันดับ 3 ที่มีสิทธิ์เข้ารอบทั้ง 3 ทีม (หนึ่งทีมต่อหนึ่งกลุ่ม) ผ่าน `identifyG16ThirdPlaceCandidates` เท่านั้น — จากนั้น Admin ที่ได้รับอนุญาต (`tournament_super_admin`) เป็นผู้**บันทึกผลลัพธ์ 2 ทีมที่ถูกจับฉลากได้จากหน้างานด้วยตนเอง** ผ่าน workflow "Manual Qualification Placeholder Assignment" (ดู PR #7, `lib/tournament/services/qualification-draws.ts`) ซึ่งเก็บ candidate pool ทั้งหมด, ลำดับที่ถูกเลือก, ผู้บันทึก (actor), เวลา, และ Audit Log ไว้ครบทุกครั้ง (append-only ผ่าน `tournament_qualification_draws`/`tournament_qualification_draw_candidates`, versioned)
- **Qualification Cutoff Tie Draw within Group (D-30) — เพิ่มเข้ามาภายหลัง (2026-07-18), แยกจาก D-07/D-29**: เมื่อทีมหลายทีม**ภายในกลุ่มเดียวกัน**คะแนนเท่ากันคร่อมเส้นโควตาเข้ารอบอัตโนมัติ (`qualify_rank_per_group`) ต้องจับฉลาก**ด้วยมือที่หน้างานเท่านั้นเช่นกัน** — ห้ามใช้ H2H/GD/GF/Fair Play/FIFA Ranking ตัดสินสิทธิ์เข้ารอบแม้เกณฑ์เหล่านั้นจะแยกอันดับแสดงผลได้ (`resolveTournamentTiebreak`/`calculateGroupStandings`'s ordering ยังใช้เกณฑ์เดิมเพื่อแสดงผลเท่านั้น) — Pure Logic แยกต่างหาก `resolveQualificationCutoff()` (`lib/tournament/standings/resolveQualificationCutoff.ts`) ตัดสิน `qualificationStatus` จากคะแนนล้วนๆ ไม่ปะปนกับ Standings Ordering — ดูรายละเอียดเต็มที่ D-30
- **Files Expected to Change**: `lib/tournament/standings/*.ts` (ไฟล์ใหม่ทั้งหมด รวม `calculateFairPlayScore.ts`, `rankCrossGroupCandidates.ts` ซึ่งมี `identifyG16ThirdPlaceCandidates`, `resolveQualificationCutoff.ts` ใหม่สำหรับ D-30), `lib/tournament/services/standings.ts` (data-loading), `lib/tournament/services/qualification-cutoff-draws.ts` (ใหม่ — D-30 Manual Draw Workflow, แยกจาก `qualification-draws.ts` เดิมของ D-29), `app/api/tournament/admin/standings/**`, `app/api/tournament/admin/qualification-draws/**` (มีอยู่แล้วจาก PR #7 — D-29 เดิม), `app/api/tournament/admin/qualification-cutoff-draws/**` (ใหม่ — D-30), `app/api/tournament/public/standings/**`
- **Database Change**: `tournament_standing_rules`, `tournament_qualification_rules`, `tournament_standing_overrides`, `tournament_qualification_draws`/`tournament_qualification_draw_candidates` จาก Phase 1 (D-29 เดิม, ไม่แตะ) — เพิ่มตารางใหม่ 2 ตารางสำหรับ D-30: `tournament_qualification_cutoff_draws`/`tournament_qualification_cutoff_draw_candidates` (Data Model §2.14c, Migration 019 — ดูเหตุผลที่ต้องเป็นตารางใหม่แยกจาก §2.14b ในหัวข้อนั้น)
- **Tests**: **บังคับ Unit Test ครบ** — head-to-head (Recursive Mini-league ตาม D-09), goal difference, Fair Play (-1/-3/-4/-5 ตาม D-06), cross-group ranking (`method='ranked'` ตาม D-07), **G-U16 Third-place Candidate Identification (`method='draw'` เฉพาะ G-U16 ตาม D-29 — ต้องคืนค่าผู้มีสิทธิ์เข้ารอบครบ 3 ทีมพอดี และห้ามเลือกหรือสุ่ม 2 จาก 3 ทีมเองเด็ดขาด)**, manual override — เป็น Test Suite ที่ V1 ไม่มีเลย (แก้ R2/R3 จาก Current State Audit)
- **Acceptance Criteria**: ทุก Tiebreak rule ตาม D-09 มี Unit Test ผ่านอย่างน้อย 1 เคสต่อกติกา; Manual Override เขียน Audit Log ทุกครั้ง; G-U16 ไม่ถูกจัดอันดับด้วยคะแนน/GD/GF สำหรับ Third-place (ใช้การจับฉลากด้วยมือที่หน้างานเท่านั้น — ซอฟต์แวร์ระบุผู้มีสิทธิ์ 3 ทีม แต่ไม่เลือกหรือสุ่มเอง)
