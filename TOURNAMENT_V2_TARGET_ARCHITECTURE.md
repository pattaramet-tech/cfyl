...
**สถานะ**: Proposal ที่ผ่าน Decision Lock บางส่วนแล้ว (2026-07-14) — ดู `TOURNAMENT_V2_DECISION_CHECKLIST.md` สำหรับ Final Decision ทุกข้อและสถานะล่าสุด จุดที่มีคำว่า **DECISION LOCKED** ในเอกสารนี้คือคำตอบสุดท้ายจากเจ้าของระบบ ส่วนที่เหลือยังเป็น Proposal รอการตัดสินใจ
...
- **DECISION LOCKED (D-01, 2026-07-14)**: Option A ยืนยันแล้ว — Supabase Project แยกจาก League อย่างสมบูรณ์

**Auth Strategy สำหรับบทบาทระดับสูง (`tournament_super_admin`, `central_control`, `venue_manager`, `match_official`)**: บัญชีรายบุคคล ผูกกับ League Identity Provider ตามแนวทางข้างต้น

**Auth Strategy สำหรับ Result-entry (DECISION LOCKED — D-03, 2026-07-14)**: ใช้ **Dedicated Shared Tournament Result-entry Account** แยกต่างหากจากรูปแบบข้างต้น — บัญชีเดียวที่เจ้าหน้าที่/ผู้ตัดสิน/ผู้ประเมินทุกสนามใช้ร่วมกันในการกรอกผลการแข่งขัน:
- **ไม่ใช่** `tournament_super_admin` — จำกัดสิทธิ์ให้ Login, เลือกสนาม, เลือก Match, กรอกผล, Save Draft, Preview, Submit และเพิ่มรายละเอียดรายงานตาม Scope ที่อนุญาตเท่านั้น
- **ห้าม**: สร้าง/ลบ Tournament, แก้กติกาตารางคะแนน, Import/Rollback โปรแกรมแข่งขัน, จัดกลุ่มใหม่, แก้ Draw Assignment, จัดการผู้ใช้, แก้ผลที่ Published แล้วโดยตรง, เข้าถึง League Administration
- Account ใช้ได้หลายสนาม แต่ผู้ใช้ต้องเลือก Venue/Match เองทุกครั้งในแอป — **Server ต้องตรวจ Match และ Venue Scope ทุก Mutation** (ยืนยันว่า `match_id` ที่ส่งมาสังกัด `venue_id` ที่อ้างจริง แม้จะไม่ปฏิเสธเพราะ Cross-venue เหมือน Role อื่น)
- **ข้อยกเว้นจากหลักการเดิม**: หมวด 11.2 เดิมกำหนดไว้ว่า "บัญชีรายบุคคลเท่านั้น ห้ามแชร์ Username/Password" — **กฎนี้ไม่ใช้กับ Result-entry Role อีกต่อไปตาม D-03** (ยกเว้นเฉพาะ Role นี้ Role อื่นทั้งหมดยังคงต้องเป็นบัญชีรายบุคคล)
- **Non-repudiation Limitation ที่ต้องชดเชยด้วย Audit Trail**: เนื่องจากเป็นบัญชีร่วม ระบบจะระบุตัวบุคคลจาก Account ID ไม่ได้ — ทุก Mutation ผ่านบัญชีนี้ต้องบันทึกอย่างน้อย: `account_id`, `session_id`, `venue_id`, `match_id`, `timestamp`, `device/browser metadata`, `before/after data`, `IP` เมื่อเหมาะสม (ดูรายละเอียดเต็มใน `TOURNAMENT_V2_DECISION_CHECKLIST.md` D-03)

---

## 6. Environment Variables

**DECISION LOCKED (D-01, 2026-07-14)**: เปลี่ยนจากแผนเดิม (คง `NEXT_PUBLIC_SUPABASE_URL` ของ League ไว้) เป็น Rename ทั้งสองฝั่งด้วย Prefix `LEAGUE_*`/`TOURNAMENT_*` อย่างชัดเจน ดังนี้:

```text
# League (Rename ตาม D-01 — ไม่คงชื่อเดิมอีกต่อไป)
LEAGUE_SUPABASE_URL
...
**หมายเหตุการเปลี่ยนแปลง**: ตัด `NEXT_PUBLIC_` prefix รูปแบบเดิมที่แยกตามผู้เรียก (client vs server) ออก ใช้ Prefix เดียวต่อระบบ (`LEAGUE_*`/`TOURNAMENT_*`) ตามคำตัดสินของเจ้าของระบบ — เมื่อเข้าสู่รอบ Implementation ต้องกำหนดว่าตัวแปรใดใน 3 ตัวต่อระบบต้องมี `NEXT_PUBLIC_` prefix เพิ่มเข้าไปจริง (เฉพาะที่ Client Component เรียกใช้ตรง) เพื่อลด surface ของ anon key ฝั่ง client — เป็นรายละเอียดระดับ Implementation ไม่ใช่การเปลี่ยนคำตัดสินเรื่องชื่อฐาน (`LEAGUE_*`/`TOURNAMENT_*`)
...
2. **Runtime Guard**: `lib/tournament/db/supabase-tournament.ts` ตรวจ `process.env.TOURNAMENT_SUPABASE_URL !== process.env.LEAGUE_SUPABASE_URL` แล้ว throw ถ้าค่าเดียวกัน (กันกรณีตั้งค่า env varผิดแล้วชี้ไป Project เดิมโดยไม่รู้ตัว — ปรับชื่อ Env Var ตาม D-01)
...
|---|---|
| Database Isolation | **Option A** — Supabase Project แยก (ดูเหตุผลเต็มหมวด 5) — **DECISION LOCKED (D-01, 2026-07-14)** |
| Auth | League Supabase เป็น Identity Provider เดียวสำหรับ Role ระดับสูง (รายบุคคล), Tournament เก็บ permission mapping ของตัวเอง; Result-entry ใช้ Dedicated Shared Account แยกต่างหาก — **DECISION LOCKED (D-03, 2026-07-14)**; v1.1 เพิ่มชั้น RBAC ระดับ Venue/Category/Match บน Tournament Project เอง (ดูหมวด 11.3) |
...
| Rollout | Tournament V2 พัฒนาเป็น Module คู่ขนานก่อน ไม่แตะ League Route/Table เลยจนกว่าจะถึง Phase Cutover (ดู Implementation Phases) — **DECISION LOCKED (D-02, 2026-07-14)**: ไม่ Migrate ข้อมูล Tournament V1 เข้า V2 เลย เริ่มข้อมูลใหม่ทั้งหมด |
| Multi-Venue (v1.1) | Venue/Court เป็น Entity จริงพร้อม RBAC Scope, ไม่ Hardcode Category↔Venue Mapping ลงโค้ด, ใช้ **Single-step Result Submission with Mandatory Preview** เป็น Default (แทนที่ Two-Step Approval เดิม — **DECISION LOCKED D-16, 2026-07-14**, ดูหมวด 11 เต็ม) |
...
|---|---|---|---|
| `tournament_super_admin` | ทั้งรายการ | จัดการทุกอย่าง: รายการ ผู้ใช้ กติกา สนาม และข้อมูลทุกส่วน | รายบุคคล |
| `central_control` | ทั้งรายการ | ดูสถานะทั้ง 4 สนาม, ตรวจ/แก้ Conflict ข้ามสนาม, อนุมัติขั้นสุดท้ายเฉพาะกรณี Correction | รายบุคคล |
| `venue_manager` | สนามที่ได้รับมอบหมาย | ควบคุม Matchday Dashboard ของสนามตน, ยืนยัน/โยกย้ายตารางแข่งภายในสนาม, Request Correction | รายบุคคล |
| `result_operator` (Result-entry) | ทุกสนาม (เลือกในแอป) | กรอกผลการแข่งขัน (Quick Result + Full Match Report) แบบ Single-step + Mandatory Preview — ดู D-03/D-16 | **Dedicated Shared Account** (ข้อยกเว้นจากหลักการข้อ 1 ด้านล่าง) |
| `match_official` | เฉพาะ Match ที่ได้รับมอบหมาย | ตรวจ/ยืนยันผลและรายงานเฉพาะนัดของตัวเอง | รายบุคคล |
| `read_only` | ตาม Scope ที่กำหนด | ดูข้อมูลอย่างเดียว แก้ไขไม่ได้ | รายบุคคล |

**หลักการบังคับ**:
1. บัญชีรายบุคคลเท่านั้น ห้ามแชร์ Username/Password ร่วมกันระหว่างเจ้าหน้าที่สนาม — **ยกเว้น Role `result_operator` (Result-entry) ซึ่งใช้ Dedicated Shared Account ตามคำตัดสิน D-03 (2026-07-14) โดยเจตนา** เพื่อลดภาระการสร้างบัญชีให้เจ้าหน้าที่/ผู้ตัดสิน/อาสาสมัครจำนวนมากในแต่ละนัด — ชดเชยด้วย Audit Trail เสริม (ดูหมวด 5)
2. Role Assignment ผูก Scope กับ `tournament_id` + `venue_id` + `category_id` (และลึกถึง `match_id` ได้สำหรับ `match_official`) — เก็บในตาราง `tournament_role_assignments` **ยกเว้น Result-entry ที่ Scope ระดับ Venue/Match ถูกเลือกในแอปต่อ Session ไม่ใช่ Fixed Scope ผูกกับ User**
3. ผู้ใช้หนึ่งคนรับได้หลาย Scope พร้อมกัน (เช่น `venue_manager` ของสนาม 1 และสนาม 2)
4. QR Code ใช้เป็น Shortcut เข้าหน้าสนามได้ แต่ยังต้องผ่าน Authentication ปกติเสมอ (QR ไม่ใช่ bypass token)
5. **Server ต้องตรวจ Scope/Match-Venue Consistency ทุกครั้ง ห้ามเชื่อ `venueId`/`categoryId` ที่ Client ส่งมาเพียงอย่างเดียว** — ทุก mutation endpoint เรียก `authorizeVenueScope(user, {venueId, categoryId, matchId})` ก่อนเขียนข้อมูลเสมอ (แก้ Production Risk R7 จาก Current State Audit โดยตรง) — สำหรับ Result-entry Role ฟังก์ชันนี้ตรวจว่า `match_id` สังกัด `venue_id` ที่อ้างจริงหรือไม่ (Consistency Check) แทนการปฏิเสธ Cross-venue เหมือน Role อื่น เพราะบัญชีนี้มีสิทธิ์ทุกสนามโดยเจตนา
6. Service Role Key อยู่ฝั่ง Server เท่านั้น ไม่ bundle ไป Client
7. ทุก Mutation สำคัญ (submit/correction) ต้องมี Audit Log พร้อม `account_id`/`session_id`/`device metadata` (สำหรับ Result-entry ดู D-03 หมวด Non-repudiation Limitation)
...
lib/tournament/services/authorizeVenueScope.ts
  ...
  → ถ้า role = result_operator (Shared Result-entry Account):
      ตรวจ Consistency ว่า matchId สังกัด venueId ที่อ้างจริงหรือไม่ (ไม่ปฏิเสธ Cross-venue)
    อื่นๆ: ตรวจ Scope ตรงตาม tournament_role_assignments ของ user นั้น (ปฏิเสธถ้าไม่ตรง Scope)
