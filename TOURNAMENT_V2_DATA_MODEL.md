...
| วิธี**จับฉลาก**เลือกทีมอันดับ 3 เมื่อ `method='draw'` | **Physical process** (จับฉลากด้วยมือบนกระดาษที่หน้างาน — ซอฟต์แวร์ไม่มีฟังก์ชันสุ่มเลือก) + **Code** (`identifyG16ThirdPlaceCandidates()` ใน `rankCrossGroupCandidates.ts` — ระบุผู้มีสิทธิ์ 3 ทีมเท่านั้น) + **Database** (`tournament_qualification_draws`/`tournament_qualification_draw_candidates`) | **ใหม่ (D-29)**: ระบบระบุ Candidate Pool 3 ทีม (หนึ่งทีมต่อกลุ่ม) ไม่จัดอันดับด้วยคะแนน และไม่เลือก/สุ่มเอง — Admin บันทึกผล 2 ทีมที่จับฉลากได้จากหน้างานผ่าน `lib/tournament/services/qualification-draws.ts` ผลลัพธ์ต้อง Persist พร้อม Audit |
...
| ทีมอันดับ 3 ที่ดีที่สุด | `tournament_qualification_rules.best_third_placed_count`/`best_third_placed_method` + `rankBestThirdPlacedTeams()` (`method='ranked'`) หรือ `identifyG16ThirdPlaceCandidates()` + การจับฉลากด้วยมือที่หน้างาน (`method='draw'`) ใน Code — **DECISION LOCKED (D-07/D-29)** |
